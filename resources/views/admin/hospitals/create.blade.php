...
<head>
    ...
    <title>{{ isset($hospital) ? 'Edit' : 'Add' }} Hospital — {{ config('app.name') }}</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=figtree:400,500,600,700&display=swap" rel="stylesheet" />

    {{-- Leaflet for coordinate picker --}}
    ...
    <style>
        ...
        .btn-ghost { display:inline-flex;align-items:center;gap:4px; background:rgba(255,255,255,0.06); color:#94a3b8; border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:8px 14px; font-size:0.875rem; text-decoration:none; transition:all 0.2s; }
...
<div class="admin-nav">
    <div class="max-w-3xl mx-auto flex items-center gap-4">
        ...
        <span class="text-sm font-semibold text-white">
            {{ isset($hospital) ? 'Edit Hospital' : 'Add Hospital / Blood Bank' }}
        </span>
    </div>
</div>
...
<div class="max-w-3xl mx-auto px-4 py-8">
    <div class="form-card">
        <h1 class="text-xl font-bold text-white mb-6">
            {{ isset($hospital) ? '✏️ Edit Hospital' : '➕ Add Hospital / Blood Bank' }}
        </h1>

        <form method="POST"
              action="{{ isset($hospital) ? route('admin.hospitals.update', $hospital) : route('admin.hospitals.store') }}">
            @csrf
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" required
                           ...
                           class="form-input" placeholder="e.g. Dhaka Medical College Hospital">
                ...
                <div>
                    ...
                    <input type="text" id="contact" name="contact"
                           ...
                           class="form-input" placeholder="e.g. 02-9999999">
                    @error('contact') <p class="error">{{ $message }}</p> @enderror
                </div>

                {{-- Coordinates with Leaflet picker --}}
                <div>
                    <label>Location on Map <span class="text-slate-600 font-normal normal-case">(click to set pin)</span></label>
                    <div id="map-picker"></div>
                    <p class="tip">Click anywhere on the map to set the latitude & longitude automatically.</p>
                    <div class="grid grid-cols-2 gap-3 mt-2">
                        <div>
                            <label for="latitude">Latitude</label>
                            <input type="number" id="latitude" name="latitude" step="0.000001"
                                   value="{{ old('latitude', $hospital->latitude ?? '') }}"
                                   class="form-input" placeholder="e.g. 23.7104">
                        ...
                        <div>
                            ...
                            <input type="number" id="longitude" name="longitude" step="0.000001"
                                   ...
                                   class="form-input" placeholder="e.g. 90.4074">
            ...
            <div class="mt-8 flex gap-3">
                <button type="submit" class="btn-primary" style="flex:1;">
                    {{ isset($hospital) ? '💾 Update Hospital' : '➕ Add Hospital' }}
                </button>
